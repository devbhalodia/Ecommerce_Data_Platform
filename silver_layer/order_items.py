...
def process_incremental_batch(microBatchDF, batchId):
    ...
    rejection_conditions = [
        (F.col("order_item_id").isNull(), "null order_item_id"),
        (F.col("order_id").isNull(), "null order_id"),
        (F.col("product_id").isNull(), "null product_id"),
        (F.col("quantity").isNull() | (F.col("quantity") <= 0), "null/zero/negative quantity"),
        (F.col("item_price").isNull() | (F.col("item_price") < 0), "null/negative item_price"),
    ]
    ...
    good_rows_df = deduped_batch_df.filter(
        ...
        (F.col("item_price").isNotNull()) & (F.col("item_price") >= 0)
    )
