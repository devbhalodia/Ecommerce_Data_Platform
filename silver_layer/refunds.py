...
def process_incremental_batch(microBatchDF, batchId):
    ...
    rejection_conditions = [
        (F.col("refund_id").isNull(), "null refund_id"),
        (F.col("order_id").isNull(), "null order_id"),
        (F.col("refund_amount").isNull() | (F.col("refund_amount") <= 0), "null/zero/negative refund_amount"),
        (F.col("refund_reason").isNull() | (F.col("refund_reason") == ""), "null/empty refund_reason"),
        (F.col("refund_timestamp").isNull(), "null refund_timestamp"),
    ]
    ...
    good_rows_df = deduped_batch_df.filter(
        ...
        (F.col("refund_timestamp").isNotNull())
    )
